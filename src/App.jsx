...
function App() {
  ...
  const totalPriceAfterDiscount = getTotalPriceAfterDiscount(
    getTotalPrice,
    ...
    <div style={{ textAlign: "center" }}>
      <Space size={20} className="my-10">
        ...
        <img
          ...
          src="/PropImg/business-icon-png-1944.png"
          alt=""
        />
      </Space>
      <div>
              ...
              <Row justify="space-between">
                <Col span={24}>
                  <div className="mx-16">
                    ...
                    <div>
                      <img style={style} src={`/img/${product.img}`} alt="" />
                    </div>
                    <div className="pt-5">
      ...
      <div className=" px-16">
        <hr class="border-t-4 border-gray-700 my-24"></hr>
      </div>
      ...
      <div className="rounded-lg outline outline-offset-2 outline-2  m-48">
        <Space size={20}>
          ...
          <img
            ...
            src="/PropImg/cart-icon-28348.png"
            alt=""
          />
        </Space>
        <ul className="pt-5">
            ...
            <li key={item.id}>
              <div className="pt-8" style={{ paddingLeft: "36rem" }}>
                <Space style={container}>
                  <div className="">
                    {item.name} - {item.price} THB {" x "}
                    {item.quantity}
                    {"  "}
                  </div>
                  <Space size={10} className="">
                    <button
                      className=" w-4 h-10 flex justify-center"
                    ...
                    <button
                      className=" w-20 h-10 flex justify-center"
        ...
        <div className="mt-16 pb-5">
          ...
          {Object.entries(discounts).map(([category, discount]) => (
            <p className="pb-5 text-violet-500" key={category}>
              Discount for {category}: - {discount} THB
            </p>
          ))}
          ...
          {isCouponApplied && (
            <div className="pb-5">
              <p className="text-yellow-400">
                คูปอง ส่วนลด 50THB: - {totalPriceAfterDiscount.disfix} THB
              </p>
            </div>
          )}
          ...
          {isCouponApplied2 && (
            <div className="pb-5">
              <p className="text-yellow-400">
                คูปอง ส่วนลด 10%: - {totalPriceAfterDiscount.dispercent} THB
              </p>
            </div>
          )}
          ...
          {inputPointstotal > 0 && (
            <div className="pb-5 ">
              <h2 className="text-green-500">
                {" "}
                ใช้ส่วนลดจาก Point - {
                  totalPriceAfterDiscount.availablePoints
                }{" "}
                THB {"(สูงสุด 20 เปอร์เซ็นต์)"}
              </h2>
            </div>
          )}
          ...
          {totalPriceAfterDiscount.Seasonal > 0 && (
            <div className="pb-5 ">
              <h2 className=" text-orange-600">
                {" "}
                ได้ส่วนลดตามฤดูกาล - {totalPriceAfterDiscount.Seasonal} THB
                (ทุกๆ 300 บาท จะได้ส่วนลด 40 บาท )
              </h2>
            </div>
          )}
          ...
          {totalPriceAfterDiscount && (
            <div className="pb-5 ">
              <p className=" text-2xl">
                Total Price after Discount: {totalPriceAfterDiscount.totalPrice}{" "}
                THB
              </p>
            </div>
          )}
        ...
        <Space size={20} className="mb-16">
          ...
          <div>My point is {Point.NumOfoint} Point</div>
        </Space>
        ...
        <Modal
          ...
          footer={[
            <Button key="cancel" onClick={hidePopuped}>
              ปิด
            </Button>,
            <Button key="ok" type="primary" onClick={handledOk}>
              ใช้
            </Button>,
          ]}
        >
          <Space>
            <div className=" text-xl">My point is {Point.NumOfoint} </div>{" "}
            <br />
            <Button key="plus" onClick={plusHandled}>
              +50
            </Button>
            <Button key="sub" onClick={subHandled}>
              -50
            </Button>
          </Space>
          <div className="pt-5">
            <Space size={20}>
              <InputNumber
                ...
                max={Point.NumOfoint}
                defaultValue={0}
                onChange={setInputPoints}
              />
              <Space size={10}>
                <h2>
                  {" "}
                  ลดไปได้ {totalPriceAfterDiscount.availablePoints} THB{" "}
                  <h3 className=" text-red-500">
                    (สูงสุด {totalPriceAfterDiscount.maxPointDiscount} THB ) Max
                    20% discount
                  </h3>
                </h2>
              </Space>
            ...
            <div>
              <h2 className="py-4 ">
                Point used last is {inputPointstotal} Point
              </h2>
              <Button
                className=" bg-slate-500 text-green-300"
                key="repoint"
                onClick={cancelpointed}
              >
                {" "}
                ยกเลิกใช้ point (คืน point)
              </Button>
            </div>
          </div>
        </Modal>
